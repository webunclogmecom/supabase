...
async function pg(sql, project, _attempt = 1) {
  ...
  try {
    ...
    if (_attempt < 5) { await new Promise(res => setTimeout(res, 2000 * _attempt)); return pg(sql, project, _attempt + 1); }
    throw _e;
  }
  ...
  if (r.status >= 300) {
    ...
    if (_retryable && _attempt < 5) {
      ...
      return pg(sql, project, _attempt + 1);
    }
    throw new Error(`PG ${project} ${r.status}: ${r.body.toString().slice(0,400)}`);
  }
