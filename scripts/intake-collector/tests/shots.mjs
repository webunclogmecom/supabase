...
const r = await fetch('https://api.supabase.com/v1/projects/wbasvhvvismukaqdnouk/database/query', { method: 'POST', headers: { Authorization: 'Bearer ' + env.SUPABASE_PAT, 'content-type': 'application/json' }, body: JSON.stringify({ query: "select token from public.property_intakes where requested_by like '[TEST] collector page check%' and submitted_at is null order by id desc limit 1" }) })
...
for (const [name, w, h, dpr] of [['phone360', 360, 740, 2], ['phone390', 390, 844, 2], ['tablet768', 768, 1024, 1], ['desktop1280', 1280, 900, 1]]) {
  const ctx = await b.newContext({ viewport: { width: w, height: h }, deviceScaleFactor: dpr, isMobile: w < 700, hasTouch: w < 700, geolocation: { latitude: 25.80941, longitude: -80.20511, accuracy: 9 }, permissions: ['geolocation'] })
