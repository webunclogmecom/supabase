...
// are pulled into their own section up top, each row's ticket# links straight to its stamped image
// (Chrome's print-to-pdf keeps <a href> as real clickable PDF links) so Yannick can click through and
// fill in the code on the image.
const fs = require('fs');
...
(async () => {
  ...
  const imgLink = (wm, page) => {
    const f = imageFor[`${wm}_${page}`];
    return f ? `file:///${path.join(dir, f).replace(/\\/g, '/')}` : null;
  };
  ...
  const rowTr = r => {
    ...
    const link = imgLink(r.white_manifest_number, r.page);
    const ticket = link ? `<a href="${esc(link)}">${esc(r.white_manifest_number)}</a>` : esc(r.white_manifest_number);
    return `<tr class="${matched ? '' : 'flag'}"><td class="ri">${r.page}.${r.row_index}</td><td>${ticket}</td><td class="code">${code}</td><td>${name}</td><td class="addr">${esc(r.address_read || '')}</td></tr>`;
  };

  const missing = rows.filter(r => !(r.assignment_status === 'matched' && r.client_code));
  const missingHtml = missing.length
    ? `<h2>Missing codes — click ticket # to open the image (${missing.length})</h2>
       <table><thead><tr><th>Row</th><th>Ticket</th><th>Code</th><th>Client</th><th>Address</th></tr></thead>
       <tbody>${missing.map(rowTr).join('')}</tbody></table>`
    : '';
  ...
  const blocks = wms.map(wm => {
    ...
    return `<div class="sheet"><div class="hd"><b>Ticket ${esc(wm)}</b> &middot; ${pages.length} page${pages.length > 1 ? 's' : ''}</div>
      <table><thead><tr><th>Row</th><th>Ticket</th><th>Code</th><th>Client</th><th>Address</th></tr></thead><tbody>${trs}</tbody></table></div>`;
  }).join('');
