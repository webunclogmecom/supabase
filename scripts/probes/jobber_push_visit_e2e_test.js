...
async function jvisit(tok, gid) { const r = await post('api.getjobber.com', '/api/graphql', JSON.stringify({ query: 'query($id:EncodedId!){ visit(id:$id){ id title startAt endAt } }', variables: { id: gid } }), { Authorization: 'Bearer ' + tok, 'Content-Type': 'application/json', 'X-JOBBER-GRAPHQL-VERSION': '2026-04-16' }); return JSON.parse(r.body).data?.visit ?? null; }
...
(async () => {
  ...
  let vid;
  try {
    // 1) create a calendar visit on 112-YA job 159
    vid = (await sql("INSERT INTO public.visits (client_id, job_id, visit_date, start_at, end_at, title, service_type, visit_status, source) VALUES (381, 159, '2026-06-25', '2026-06-25T13:00:00Z'::timestamptz, '2026-06-25T14:00:00Z'::timestamptz, '01 - Service Agreement - Pumping - Grease Trap & Tank Cleaning', 'GT', 'scheduled', 'visit-calendar') RETURNING id"))[0].id;
    console.log('[e2e] inserted calendar visit id=' + vid);
    ...
    if (gid) console.log('[e2e]   jobber visit:', JSON.stringify(await jvisit(tok, gid)), '(expect 09:00-10:00 ET = 13:00-14:00Z)');

    // 3) MOVE DATE -> 06-26 11:00 ET
    await sql("UPDATE public.visits SET visit_date='2026-06-26', start_at='2026-06-26T15:00:00Z'::timestamptz, end_at='2026-06-26T16:00:00Z'::timestamptz WHERE id=" + vid);
    r = await callFn('upsert', vid);
    console.log('[e2e] UPDATE push ->', r.status, r.body);
    if (gid) console.log('[e2e]   jobber visit after move:', JSON.stringify(await jvisit(tok, gid)), '(expect 2026-06-26 11:00 ET = 15:00Z)');
