...
(async () => {
  console.log('=== Visit 1713 (the one Yannick is looking at) ===');
  const visit = await pg(`
    SELECT v.id, v.title, v.visit_date::text, v.service_type AS canonical_service_type,
      v.start_at AT TIME ZONE 'America/New_York' AS start_et,
      v.job_id, c.client_code, c.name AS client_name, c.status AS client_status
    FROM visits v
    LEFT JOIN clients c ON c.id = v.client_id
    WHERE v.id = 1713;`);
  console.log(JSON.stringify(visit, null, 2));

  console.log('\n=== Line items on job 268 (what Lovable is reading) ===');
  const li = await pg(`
    SELECT li.id, li.name AS line_item_name, li.description, li.quantity, li.unit_price, li.total
    FROM line_items li WHERE li.job_id = 268 ORDER BY li.id;`);
  console.log(JSON.stringify(li, null, 2));

  console.log('\n=== service_configs for client 042-MT (our canonical service-type record) ===');
  const sc = await pg(`
    SELECT sc.service_type, sc.frequency_days, sc.price, sc.equipment_size_gallons
    FROM service_configs sc JOIN clients c ON c.id = sc.client_id
    WHERE c.client_code = '042-MT' ORDER BY sc.service_type;`);
  console.log(JSON.stringify(sc, null, 2));

  console.log('\n=== Airtable: 042-MT canonical Service Type ===');
  ...
  console.log({
    ...
    'Client Code #3': at?.['Client Code #3'],
    'ACTIVE/INACTIVE': at?.['ACTIVE/INACTIVE'],
    'Service Type': at?.['Service Type'],
    'GT Frequency': at?.['GT Frequency'],
  });

  console.log('\n=== How often does any commercial client have "Residential" in their line_items? ===');
  const widespread = await pg(`
    SELECT COUNT(DISTINCT v.id) AS n_visits,
  ...
  console.log('\n=== Most common line_item names overall (Top 15) ===');
  const top = await pg(`
    SELECT name, COUNT(*) AS n FROM line_items GROUP BY name ORDER BY n DESC LIMIT 15;`);
