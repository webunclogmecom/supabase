...
--
-- ⚠ SIGNATURE, and this file has already been broken once by not tracking it:
--     fn_record_manual_gdo_report(visit_id, attempted_at, run_id, screenshot_path,
--                                 filed_by_email, gdo_id DEFAULT NULL)
-- `p_confirmation` was REMOVED on 2026-08-20 (the county issues no confirmation number; the
-- function writes the sentence itself). The old 7-argument calls in this file failed with
-- "function does not exist", which the suite reported as **"FAIL 7: the happy path was refused"** -
-- i.e. broken tooling wearing the costume of a broken product. If you change the signature again,
-- fix this file in the SAME commit.
-- ============================================================================
DO $suite$
DECLARE
  v_visit bigint; v_manifest bigint; v_bucket text;
  v_run text := 'manual-suiteprobe';
  v_path text;
  err text;

  r_happy bool; r_nopath bool; r_badrun bool; r_future bool;
  r_wrongvisit bool; r_dupe bool; r_failedok bool; r_nolink bool;
  q_before bool; q_after bool; q_permanent bool;
  v_written text;
  c_expected constant text := 'Filed manually, DERM portal confirmed (no tracking number)';
BEGIN
  -- A post-cutoff, manifest-linked visit we can safely reshape inside this transaction.
  ...
  BEGIN
    PERFORM public.fn_record_manual_gdo_report(v_visit, now()-interval '1 hour', v_run, v_path, 'fred@ayache.com');
    r_nopath := true;
  ...
  -- ---- 3. rejected inputs, each with the object present so only the named rule can fire --------
  BEGIN PERFORM public.fn_record_manual_gdo_report(v_visit, now()-interval '1 hour','rpa-notmanual',v_path,'fred@ayache.com');
    r_badrun := true; EXCEPTION WHEN others THEN r_badrun := false; END;
  BEGIN PERFORM public.fn_record_manual_gdo_report(v_visit, now()+interval '30 days', v_run, v_path,'fred@ayache.com');
    r_future := true; EXCEPTION WHEN others THEN r_future := false; END;
  BEGIN PERFORM public.fn_record_manual_gdo_report(v_visit, now()-interval '1 hour', v_run,'999999/'||v_run||'.jpg','fred@ayache.com');
  ...
  -- ---- 5. the CLIENT-FACING sentence is written by the function, not by the caller -------------
  -- customer.gdo_reports exposes portal_confirmation to the Field Portal, so this string is read by
  -- paying clients. It is asserted here because no caller can supply it any more.
  SELECT portal_confirmation INTO v_written
    FROM public.derm_portal_submissions WHERE visit_id = v_visit AND run_id = v_run;

  -- ---- 6. it must now be OUT of the queue, permanently ----------------------------------------
  ...
  -- ---- 7. a second filing for the SAME permit must be refused ----------------------------------
  BEGIN PERFORM public.fn_record_manual_gdo_report(v_visit, now()-interval '1 hour', v_run||'b', v_path,'fred@ayache.com');
    r_dupe := true; EXCEPTION WHEN others THEN r_dupe := false; END;

  -- ---- 8. a FAILED bot attempt must NOT block a manual filing ----------------------------------
  DELETE FROM public.derm_portal_submissions WHERE visit_id = v_visit;
  ...
  -- ---- 9. a post-cutoff visit with NO manifest link is ACCEPTED (block removed 2026-08-19_1915)
  -- An unlinked visit has no row in v_derm_portal_fields, so it was never in the bot's queue and
  -- there is nothing to stop. The view reports suppresses_bot = false instead.
  DELETE FROM public.derm_portal_submissions WHERE visit_id = v_visit;
  ...
  RAISE NOTICE ' 4 future filing date          -> accepted %  (want f)', r_future;
  RAISE NOTICE ' 5 path for another visit      -> accepted %  (want f)', r_wrongvisit;
  RAISE NOTICE ' 6 HAPPY PATH                  -> accepted %  (want t)  [%]', r_happy, coalesce(err,'-');
  RAISE NOTICE ' 7 confirmation WRITTEN by fn  -> %', coalesce(v_written,'(null)');
  RAISE NOTICE ' 8 in queue AFTER filing       -> %           (want f)', q_after;
  RAISE NOTICE ' 9 excluded PERMANENTLY        -> %           (want t)', q_permanent;
  RAISE NOTICE '10 second filing, same permit  -> accepted %  (want f)', r_dupe;
  ...
  IF r_future     THEN RAISE EXCEPTION 'FAIL 4: a future filing date was accepted'; END IF;
  IF r_wrongvisit THEN RAISE EXCEPTION 'FAIL 5: a screenshot path for another visit was accepted'; END IF;
  IF NOT r_happy  THEN RAISE EXCEPTION 'FAIL 6: the happy path was refused: %', err; END IF;
  IF v_written IS DISTINCT FROM c_expected THEN
    RAISE EXCEPTION 'FAIL 7: the client-facing sentence changed. expected "%", got "%"', c_expected, coalesce(v_written,'(null)');
  END IF;
  ...
  IF r_dupe       THEN RAISE EXCEPTION 'FAIL 10: a duplicate filing for the same permit was accepted'; END IF;
  IF NOT r_failedok THEN RAISE EXCEPTION 'FAIL 11: a failed bot attempt still blocks the manual path: %', err; END IF;
  IF NOT r_nolink THEN RAISE EXCEPTION 'FAIL 12: an unlinked visit is blocked - Fred asked for every visit'; END IF;
