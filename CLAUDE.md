...
Single source-of-truth Postgres warehouse for Unclogme LLC on Supabase project `wbasvhvvismukaqdnouk`. Webhooks from Jobber and Samsara land in Edge Functions and normalize into a 28-table 2NF/3NF schema. The third handler, `webhook-airtable`, is retired: Airtable was fully retired 2026-07-24 and is no longer a live inbound feed. Cross-system IDs live in one polymorphic bridge table (`entity_source_links`) — never as source-prefixed columns. Jobber + Airtable visit-gen sunset May 2026 (visit-gen already done 2026-05-13). **DERM capture moved to the DERM Tracker app** — it writes `derm_manifests` directly to Supabase; Airtable is retired for DERM, and as of **2026-07-21 that is finally true in code**: `'derm_manifest'` was removed from `ENTITY_TO_HANDLER` in `webhook-airtable` (Fred's explicit call). ⚠ **The 2026-06-26 "verified" note was wrong and is a lesson worth keeping**: it checked that *recent inserts* carried `app_source='derm-tracker'`, which they did, and concluded the feed was dead. The handler was still wired and still fired **55 times in the 30 days to 2026-07-14**, inserting manifests with a NULL `disposal_facility_id` (a column that appeared nowhere in that file) and wiping dump dates via null-valued keys in its `.update(row)`. **Absence of recent bad rows is not absence of a live writer** (they had been backfilled away). To retire a feed, cut the dispatch and prove a synthetic event returns `skipped`, don't infer it from data. Severing also ended Airtable PDF doc-mirroring (accepted: 545/546 live manifests already mirrored, 0 point at Airtable); stray events now land in `webhook_events_log` as `status='skipped'` rather than vanishing. **⚠ AIRTABLE IS FULLY RETIRED — we do not use Airtable anymore (Fred, 2026-07-24).** Never cite an "Airtable / AT sunset" as a reason to defer or gate ANY work — that gate no longer exists. 🛑 **BUT "never read it" IS TOO BROAD AND IS CORRECTED BELOW (2026-09-23): there is exactly ONE live Airtable surface left, the PRE/POST SHIFT INSPECTIONS table.** Airtable is not a source for client, visit, job, quote, invoice or DERM manifest data — all of that moved, and a stale Airtable answer there would be wrong. Shift inspections are the documented exception. Its last inbound feed, PRE-POST `inspections` via `webhook-airtable`, has gone quiet (last DB write **2026-07-14**; inbound events stopped ~2026-07-15) — **the FEED is dead. 🛑 THE DATA IS NOT, AND "treat it as dead, not live" WAS READ AS THE SECOND AND BROADCAST AS FACT ON 2026-09-23.** Corrected by Fred the same day and then VERIFIED against the live base: **pre/post SHIFT inspections are still being collected every shift, through two Fillout forms, into Airtable `appjMgjjZPeuudqQR/tblDklRdntb6kdjvE`.** Newest record when checked was **2026-09-23 01:25 UTC** (Michael E, Moises 3800, Pre Inspection); **443 rows there against 319 in `public.inspections`**, i.e. ~124 inspections our warehouse has never seen. ⇒ **What died is the PIPE from Airtable into Supabase. The upstream never stopped.** So `public.inspections` was stale and incomplete, and Airtable is still the system of record for shift inspections. **✅ THE OPEN QUESTION IS ANSWERED AND SHIPPED (2026-09-23): the two Fillout forms now post to a SECOND destination as well, the edge function `fillout-inspection`, so a shift inspection reaches the warehouse on submission.** Airtable is untouched and keeps receiving exactly what it always did; this is an addition, never a replacement. ✅ **AND THE HISTORY WAS BACKFILLED THE SAME DAY** (Fred: *"Go ahead first with the backfill"*): `public.inspections` went **319 -> 425** via `scripts/migrate/airtable_inspection_backfill.js`, keyed on the Airtable record id in `entity_source_links` so a re-run imports nothing, plus 1,585 attachments mirrored by `scripts/migrate/airtable_inspection_attachments.js`. **19 of the 444 Airtable records are deliberately still out**: 10 have no Pre/Post answer (3 blank rows, 7 real May/June 2025 inspections from before the form asked) and 9 are a driver redoing a form for a shift we already hold. ⚠ **75 rows written before that day carry a UTC-sliced `shift_date` where the operating-date rule says the ET clock date** (100% of legacy rows match UTC, 100% of the backfilled ones match ET). Re-dating them is OPEN with Fred: `public.inspections` has no audit trigger and 6 shift groups would collide on `idx_inspections_shift_unique` once moved. Read [docs/reference/fillout-inspection-intake.md](docs/reference/fillout-inspection-intake.md) before touching either form, the function or the photo-role vocabulary. ⚠ This is the estate's own correct-measurement-wrong-sentence trap, committed by the file that warns about it: every number in the original sentence was right. (The Admin Review app remains the inspections review surface, reading canonical Supabase, not Airtable.) `derm_manifests` were already severed from Airtable in code 2026-07-21. **Odoo.sh is DROPPED (Fred 2026-07-08)** — CRM/client management moves to in-house apps (the planned "Client App" is slated to become the client-data master; Jobber sunset expected ~Aug-Sep 2026); Samsara is permanent.
...
- **Git hooks live in `.githooks/` and are enabled per clone with `git config core.hooksPath .githooks`**
  (done on this machine 2026-09-18). Today there is one, `pre-commit`: it refuses a commit that touches
  `postman/README.md` or the Postman collection while the collection's generated `info.description` is
  stale, and prints the one command that fixes it (`node scripts/postman/sync_collection_description.js`).
...
4. **The intake token must never be copied into a path, a caption or any column staff can read.** It is
   ...
   adversarial review found the copy in `storage_path` before it shipped (2026-09-23). A second round
   then found it in **`audit.logs`**, which copies whole rows and which `authenticated` can SELECT, so
   `property_intakes.token` is now in `audit.redacted_columns`. A secret is as exposed as the
   least-protected column that copies it: ask where it gets copied, not who can read its table.
5. **Completeness is ONE rule: `public.fn_intake_missing`. Call it, never re-implement it.** A requested
