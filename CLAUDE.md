...
**⚠ `clients.status` itself is NOT authoritative for "recurring."** It flip-flops via competing sync writers — an Airtable-`Recurring` mirror vs an ACTIVE-reset (True Barista's status ping-ponged ACTIVE↔RECURRING ~7× in May 2026, landing RECURRING by chance). **Authoritative rule (Fred 2026-07-15): a client is recurring ONLY if it's on Yannick's SA-build list OR Fred set it explicitly.** Marked `Recurring` in AT / `clients.status` but absent from that list ⇒ a *discrepancy*, not a recurring client (e.g. True Barista 209/212/213-TRUE — no SA job, one-off visits only).
