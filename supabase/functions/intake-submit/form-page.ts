...
<style>
:root{--bg:#f6f6f7;--card:#fff;--ink:#18181b;--mut:#6b7280;--line:#e4e4e7;--red:#f14714}
...
function field(q){
  ...
  if(t==='yes_no'){
    ...
    var f=document.createElement('input'); f.type='file'; f.accept='image/*'; f.setAttribute('capture','environment'); f.multiple=true; f.style.display='none';
    f.onchange=function(){ var fs=[].slice.call(f.files||[]); f.value=''; fs.forEach(function(file){up(q.key,file)}) };
    var fb=document.createElement('button'); fb.type='button'; fb.className='btn'; fb.textContent=list.length?'Add more photos':'Take or add photos'; fb.onclick=function(){f.click()};
