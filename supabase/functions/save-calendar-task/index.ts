...
//
// ============================================================================================
// 🛑 WHAT THE READ-BACK CANNOT VERIFY — measured live 2026-08-26, read-only.
// ============================================================================================
// AN ALL-DAY JOBBER TASK EXPOSES NO DATE AT ALL. Sampled 100 real production Tasks: all 3 with
// allDay=true return startAt: null, endAt: null, duration: null, while 0 of the 97 timed ones have
// a null startAt. The correlation is perfect, so this is the type's behaviour and not a data gap.
// ⇒ For an all-day task the read-back can confirm allDay, title, instructions, isComplete, client,
//   property and assignees — but NOT task_date. That one field is ours alone and Jobber will never
//   confirm or contradict it. Named here because it is the one hole in "no discrepancies", and it
//   is a property of Jobber's API, not something this function can close.
// ⇒ startAt is still SENT on an all-day create (ET midnight of task_date). ET midnight lands at
//   04:00/05:00 UTC on the SAME calendar day, so it reads as the right date whether Jobber
//   interprets it in the account's timezone or in UTC. Whether it actually places the task on that
//   day is UNVERIFIED and needs the live create in Task 8.
//
...
//
// Jobber ops, all proven live 2026-08-25 (docs/specs/2026-08-25-calendar-tasks-jobber-findings.md):
...
type TaskWant = {
  title: string; allDay: boolean; startAt: string; endAt: string;
  clientGid: string | null; propertyGid: string | null; assignedGids: string[];
};
function verifyTask(t: any, want: TaskWant): string[] {
  ...
  // 🛑 The time window is only checkable on a TIMED task. An all-day Jobber Task returns
  // startAt: null BY DESIGN (measured on 3 of 3 live all-day tasks, against 0 of 97 timed ones),
  // so asserting on it here would fail EVERY all-day save. task_date on an all-day task is
  // therefore unverifiable through Jobber — see the header. This is the one gap, and it is named
  ...
  if (want.clientGid && t.client?.id !== want.clientGid) bad.push("client did not attach");
  if (want.propertyGid && t.property?.id !== want.propertyGid) bad.push("property did not attach");
  if (want.assignedGids.length) {
    const got = new Set((t.assignedUsers?.nodes ?? []).map((u: { id: string }) => u?.id));
    const absent = want.assignedGids.filter((g) => !got.has(g));
    if (absent.length) bad.push(`${absent.length} assignee(s) did not attach`);
  }
  return bad;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (op !== "create") {
      ...
      const { data: asg, error: aErr } = await db.schema("ops").from("calendar_task_assignees")
        .select("employee_id").eq("task_id", taskId);
      if (aErr) return fail(500, "db_error", `assignee read failed: ${aErr.message}`);
      curAssignees = (asg ?? []).map((a: { employee_id: number }) => Number(a.employee_id));
    }
    ...
    if (op === "delete") {
      ...
      const res = await gql(token, M_TASK_DELETE, { ids: [link!.source_id] });
      const errs = errsOf(res, "taskDelete");
      if (errs.length) {
        return fail(502, "jobber_rejected", `Jobber refused the delete: ${errs.join("; ")}`,
          { jobber_errors: errs });
      }

      // 🛑 REQUIRE POSITIVE PROOF THE TASK IS GONE. Dropping our row on a response we cannot read
      // is unrecoverable: entity_source_links carries NO audit trigger, so once the link is gone
      // the GID exists nowhere in our system and the Task lives on the crew's schedule forever.
      const check = await gql(token, Q_TASK, { id: link!.source_id });
      if (!answered(check)) {
        return fail(502, "jobber_unverified",
          "Jobber did not return an answer, so the task may still exist. Nothing was deleted on our side — try again once Jobber is responding.",
          { jobber_task: link!.source_id });
      }
      if (check.data.task?.id) {
        return fail(502, "jobber_unverified",
          "Jobber reported no error but the task is still there. Nothing was deleted on our side.",
          { jobber_task: link!.source_id });
      }
      ...
      return json({
        ...
        verified_gone: true, row_removed: removed === true,
      });
    }
    ...
    if (has(body, "assignee_ids")) {
      ...
      assigneeIds = curAssignees;                          // an edit re-states what is already there
    }

    let assignedGids: string[] = [];
    ...
    // ET midnight for an all-day task: it lands at 04:00/05:00 UTC on the SAME calendar day, so it
    // reads as the right date under either interpretation Jobber might apply.
    const startAt = etToUtcISO(taskDate, allDay ? 0 : (minutes as number));
    const endAt = new Date(new Date(startAt).getTime() + (allDay ? 1439 : duration) * 60_000).toISOString();
    ...
    // dispatcher set by hand in Jobber.
    if (assignedGids.length) input.assignedTo = assignedGids;

    let gid = link?.source_id ?? "";

    if (op === "edit") {
      // taskEdit carries clientId/propertyId INSIDE the input — the mirror image of taskCreate.
      if (clientGid) input.clientId = clientGid;
      if (propertyGid) input.propertyId = propertyGid;
    ...
    const mismatches = verifyTask(t, { title, allDay, startAt, endAt, clientGid, propertyGid, assignedGids });
    ...
    if (mismatches.length) {
      ...
      if (op === "create") {
        const del = await gql(token, M_TASK_DELETE, { ids: [gid] });
        rolledBack = answered(del) && errsOf(del, "taskDelete").length === 0;
      }
      return fail(502, "jobber_unverified",
    ...
    if (rpcErr) {
      ...
      const mapped = mapRpcError(rpcErr);
      console.error(`[calendar-task] RPC failed on ${op}: ${mapped.message}; rolledBack=${rolledBack}`);
      return fail(mapped.status, mapped.code,
        op === "create"
    ...
    return json({
      ...
      // Stated rather than implied: an empty list did NOT clear the Jobber assignment (see header).
      assignees_cleared: false,
      // Said out loud on every all-day save, because it is the one thing the verification below
      // could not check.
      date_verified_in_jobber: !allDay,
      verified: true,
    });
  } catch (e) {
    ...
    console.error(`[calendar-task] unhandled: ${msg}`);
    return fail(500, "unexpected", `Something went wrong and nothing was saved: ${msg}`);
  }
});
