# Session audit, 2026-08-20 to 2026-08-21
...
| SC-job plan, Tasks 4 to 14 | `912bb3c` `3fb912c` `04a2382` `cab99e1` `33eef99` `943ee8c` `3ba975e` `55ac4b9` |
...
### 3a. A case-sensitive check manufactured 36 findings out of nothing

My first assertion flagged **51 "decorated" Service Call titles** using
`btrim(title) <> 'Service Call'`, which is case-SENSITIVE. The real matcher in
`_shared/service-call-job.ts` is `String(t).trim().toLowerCase() === 'service call'`, i.e.
case-INSENSITIVE.

```
titles the REAL matcher accepts   643
differ only by case               36   <- my false positives, the matcher ACCEPTS these
...
`\b`-vs-`\y` regex trap and the `["']`-only bundle matcher already recorded in the estate's notes.

**The residual 15 are real but not mine**: `"Service call - 341"`, `"service call (one) [OLD]"` and
similar, created 2026-04-29 through 2026-08-19. The matcher rejects them, so `ensureServiceCallJob`
would not recognise them and could create a second job for that property. Practical risk today is
low (the helper only runs on freshly created properties, which have no jobs), but it is a genuine
edge worth knowing. **Not fixed here**, because renaming live job titles is a data change needing
Fred.
...
| 6 | **390px mobile visual on the Calendar** | Never done, because `resize_window` cannot shrink a maximized Chrome |
