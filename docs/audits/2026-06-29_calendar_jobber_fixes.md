...
**Follow-up `2026-06-29e` (gap found by Fred): the GENERATOR fix missed the manual create path.** Visits made via the Calendar "New Visit" form go through `create_calendar_visit` (source `visit-calendar`), which inserts `p_title` verbatim — so a manually-created SC visit (Fred's 168-AVA "Service Call") reached Jobber with no prefix. Fixed at the DB level instead of per-path: a `BEFORE INSERT` trigger `trg_prefix_visit_title` client-prefixes the title for any `visit-calendar`/`supabase_cron` insert (guard skips already-prefixed; inbound `jobber` untouched). Backfilled 3 stragglers (6835/6808 SC + 1 cron). **Smoke test passed** (bare "Service Call" → "168-AVA AVA - Service Call" on insert) and **6835 verified live in Jobber**. The generator's JS prefix is now redundant-but-harmless.
