...
## Part 1: Fix 2 service-config frequencies in Airtable
...
## Part 2: Review 12 abandoned clients — activate or mark INACTIVE/PAUSED
...
- **140-TYO Tacos Yoyo — Airtable has wrong client_code (140-TCY).** Jobber and our DB correctly say `140-TYO`. Fix Airtable to match. (Reminder: clients are sourced from Jobber, not Airtable — so we're not affected, but Airtable views will be wrong until fixed.)
...
- **201-ALA Aladdin** — residential site (recurring meetings, not standard service). Excluded from Part 2. Note: 201-ALA's visits aren't reaching our DB from Jobber — separate sync issue, will investigate.
