...
| 🛑 `G14_SPANS_EXTRA_SLOTS` refuses the correct fix | it counts printed rows through `v_sheet_printed_rows`, generated-sheet only, so on a handwritten pad it falls back to 1 and forbids a multi-row client's real band. Same root cause as the `expected_slots` fix, second consumer. **Not relaxed**: it guards the leak direction |
