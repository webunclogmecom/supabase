...
**Status:** Edge Function **`supabase/functions/jobber-push-visit`** built + deployed (verify_jwt=true; auth = service_role `role` claim) + **E2E-tested on 112-YA** (create / move-date / delete all ✅, cleanup ✅). Runner: `scripts/probes/jobber_push_visit_e2e_test.js`. Flag table: `docs/migrations/2026-06-01_visit_sync_flags.sql`.
**Realtime trigger LIVE + tested** (`docs/migrations/2026-06-01_jobber_push_trigger.sql`): `trg_push_visit_insert/update` on `public.visits WHERE source='visit-calendar'` → `fn_push_visit_to_jobber()` (SECURITY DEFINER) → `net.http_post` → the Edge Function. Service key in **Vault** (`jobber_push_service_key`). Auto-fired + linked in ~2s in test (`scripts/probes/jobber_push_trigger_test.js`). pg_net enabled. Form support: `visits.service_line_item_id` + `ops.client_jobs` (anon-readable). **Remaining:** webhook-jobber loop-guard (don't clobber `visit-calendar` visits — needed before the form goes live), the Calendar form change (Lovable), cron backstop.
