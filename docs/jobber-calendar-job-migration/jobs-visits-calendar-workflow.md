...
| `public.create_calendar_visit(...)` (RPC) | **atomic** create: inserts the visit (`source='visit-calendar'`, `visit_status='scheduled'`, derived `service_type` + `derm_required`, job's `property_id`) + visit-scoped `line_items` + `visit_locations` (form choice overrides the auto-seed). Stores `p_vehicle_id` (truck) **and `p_driver_id` → `visits.assigned_driver_id`** (planned driver, added 2026-06-24 `2026-06-24_visit_assigned_driver.sql`). SECURITY DEFINER (`line_items` has no anon INSERT). Migrations `2026-06-23_create_calendar_visit_rpc.sql` + `2026-06-24_visit_assigned_driver.sql`. |
| Driver on a visit | Two concepts: **actual** driver = derived from `visit_assignments` (Samsara attribution, set once driven — populates `v_calendar_visit.driver_id` for completed visits); **planned** driver = `visits.assigned_driver_id`, set on the Calendar form. The view exposes `driver_id = COALESCE(actual, assigned)` (+ `assigned_driver_id`/`assigned_driver_name`), so scheduled visits show the assigned driver and completed keep the real one. Supersedes the older "trucks-only, never a person" rule (Building Apps CLAUDE.md #4 — stale; person-driver was already tracked via `visit_assignments`). |
| `ops.create_calendar_visit(...)` (wrapper) | so the app's ops-schema client can call it. Migration `2026-06-23_ops_calendar_form_exposure.sql`. **⚠ Every ops calendar RPC (`create_calendar_visit`, `edit_calendar_visit`, `delete_calendar_visit`, `ripple_reschedule_visit`) is a thin `ops.<fn>` wrapper over `public.<fn>` — the app calls the `ops` copy. RULE: any change to a `public.<fn>` signature MUST recreate the matching `ops.<fn>` wrapper in the SAME migration, and DROPs must be schema-QUALIFIED (never a name-only `DO`-block like `WHERE proname='…' AND pronargs<>N` — that reaches across schemas and silently nukes the ops wrapper). This broke the Calendar 2026-07-02→03 ("Could not find the function ops.create_calendar_visit(...) in the schema cache") — fixed by `2026-07-03_restore_ops_create_calendar_visit.sql`. Guard: run `node scripts/probes/check_ops_app_rpcs.js` after any calendar-RPC migration (asserts the ops RPCs exist + mirror public arg counts).** |
