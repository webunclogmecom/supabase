...
| `public.rewrite_job_line_items` | `(p_job_id bigint, p_lines jsonb) → void` | Atomic, advisory-locked per-job rewrite of a job's job-scope line items (kills the concurrent delete-then-insert duplication race). All three inbound writers route through it. Migration `2026-09-01_1620`. Empty array clears. service_role only since `2026-09-26_0018` (authenticated held EXECUTE until then). |
| `public.fn_jobber_resolve_job` | `(p_gid text, p_job_number text DEFAULT NULL, p_job_status text DEFAULT NULL) → TABLE(entity_id bigint, was_created boolean)` | Atomic find-or-create of a job by Jobber GID (per-GID advisory lock, re-read after the lock; the key is shared with `fn_record_client_job`). The shell row carries job_number AND job_status, so `jobs_active_job_number_uniq` refuses a live recycled number inside the function (shell + link roll back together); an archived import still succeeds; NULL status = the pre-2026-09-26 behaviour. The index is `WHERE job_number IS NOT NULL AND job_status NOT IN ('archived','destroyed')` since `2026-09-26_0358` (a job Jobber deleted frees its number; `closed` is still covered). SECDEF, `search_path=''`, service_role only. Migrations `2026-09-09_1230`, `2026-09-26_0313`, `2026-09-26_0358`. |
| `public.rewrite_invoice_line_items` / `public.rewrite_visit_line_items` | `(p_invoice_id \| p_visit_id bigint, p_lines jsonb) → void` | The same for invoice- and visit-scoped lines, used by webhook-jobber's handleInvoice / handleVisit (Jobber delivers webhooks in pairs; the old two-request delete-then-insert duplicated lines). Delete predicate `invoice_id = X` / `visit_id = X`, rows in array order, values as given. p_lines must be a JSON array (empty clears); anything else raises 22023. SECDEF, service_role only. Migration `2026-09-25_2350`. |
...
| notes | 1,853 | Jobber migration complete 2026-04-21. 81% visit-scoped, 19% client-scoped. |
| photos | 8,019 | Jobber migration complete. 7,819 images + 176 videos + 24 PDFs = 9.3 GB in bucket `GT - Visits Images`. |
| photo_links | 8,019 | Polymorphic bridge. 7,066 to visit, 953 to note. |
| entity_source_links | 19,989+ | Cross-system. Grew ~10k from the Jobber migration (notes + photos). |
| jobber_oversized_attachments | 35 | Files > 50 MB that couldn't fit Supabase Pro bucket cap. Tracked for later recovery. |
