...
def build_digest(date_label, phase, filed, review, failed, queue, last_run, collapse_queue=False):
    ...
    phase      : "live" or "dry"
    filed      : [{"code","name","gdo_number","manifest_number","time"}]  # manifest_number = white_manifest_number
...
def classify(results, queue_items):
    ...
    for r in results:
        ...
        if r["status"] == "SUCCESS":
            # gdo_number + white_manifest_number come from the queue report the bot just filed;
            # portal_confirmation is still POSTed to the API for the audit log, just not shown here.
            filed.append({**row, "gdo_number": r["gdo_number"], "manifest_number": r["white_manifest_number"],
                          "time": to_et(r["attempted_at"])})
