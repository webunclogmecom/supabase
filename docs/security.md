...
**anon is now WRITE-NONE on `visits` + `properties`** — fully read-only on business data. `authenticated` retains direct write on `visits.{manhole_count,derm_required}` + `properties.{grease_trap_manhole_count,sample_port_count}` (Admin Review / DERM app writes, post-login); all other visit mutation is RPC-only (`set_visit_status` + the 6 lifecycle RPCs, authenticated). The now-moot anon UPDATE RLS policies (`anon_update_visit_derm_required`, `visits_calendar_update`, `properties_anon_update_manhole_authn`) are harmless dead policies — column privilege blocks anon before RLS is reached; optional cleanup.
