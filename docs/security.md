...
**Accepted residual (Phase 1 shipped 2026-07-09, full close pending Phase 2/3):** anon can still set `visit_status='cancelled'` (must stay grantable for the Calendar cancel/complete flow) → Jobber delete, and call the intentionally-anon SECDEF RPCs. The 3-phase close is planned in `docs/handoffs/2026-07-09_visit_lifecycle_rpc_and_auth_PLAN.md`:
- **Phase 1 DONE** (`2026-07-09_set_visit_status_rpc.sql`): guarded `public.set_visit_status(visit_id, status, reason)` — SECDEF, **GRANT EXECUTE `authenticated` only** + in-body domain guard (`@ayache.com`/`@unclogme.com`) — owns complete/uncomplete/skip/cancel (cancel==skip in Jobber: both delete one occurrence + promote the next SA occurrence via the shared `fn_promote_next_sa_occurrence`). Additive: current grants untouched, so no app breaks. Also closed a real anon→Jobber-delete vector found in review: `fn_request_jobber_push` PUBLIC EXECUTE grant REVOKEd (now postgres/service_role only). 16/16 smoke-verified (rollback-isolated; zero prod change).
- **Phase 2** (Building Apps): migrate the Calendar lifecycle writes onto `set_visit_status`; put the 4 staff apps behind Supabase Auth (domain-restricted signup). Field Portal stays anon.
- **Phase 3** (after Phase 2 live): `REVOKE UPDATE (visit_status, completed_at)` + revoke anon EXECUTE on `skip_visit`/`unskip_visit`/`delete/edit/ripple/create_calendar_visit` → RPC-only; re-run `rls_verify.js` (anon cancel/skip/delete must 42501). ⚠ Phase-2 PREREQ: disable open email/password signup or enforce confirmation, else the domain guard is bypassable by self-registering an `@unclogme.com` account.
