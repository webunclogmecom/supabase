...
- **Column-level `GRANT UPDATE`** to anon + authenticated on `manhole_count, derm_required` (Admin Review manhole, DERM Tracker toggle). `REVOKE UPDATE/INSERT` otherwise. **⚠ Phase 3 (2026-07-11) REVOKEd `visit_status` + `completed_at` from anon+authenticated** — those were in this set at 2026-07-09 lockdown but lifecycle is now RPC-only via `set_visit_status`; the remaining anon/authenticated direct-write surface on `visits` is `manhole_count` + `derm_required` **only**. PostgREST → **SQLSTATE 42501 / HTTP 403, atomic** on any other column (deleted_at, visit_date, start/end_at, source, sync_state, job_id, client_id, title, …). RLS can't compare OLD/NEW, so column privileges — not a `WITH CHECK` — do the column scoping.
