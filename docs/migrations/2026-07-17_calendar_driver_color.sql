-- 2026-07-17_calendar_driver_color.sql
-- Expose the canonical per-driver colour to the Calendar, so it can stop hashing crew colours and
-- render the SAME colour the DUMP app uses (Fred, 2026-07-17: "update the Calendar to read
-- employees.color_hex").
--
-- Source: public.employees.color_hex (added 2026-07-17_employees_color_hex.sql). The Calendar today
-- computes an avatar colour by hashing the driver name into a Tailwind palette at runtime, so it does
-- not match the DUMP app. These two additive view columns hand the frontend the real colour.
--
-- ops.v_calendar_visit.driver_color — the colour of the EFFECTIVE driver shown on the visit chip.
--   ⚠ It follows the SAME source as driver_id/driver_name: CASE WHEN emp.id IS NOT NULL THEN
--   emp.color_hex ELSE asg.color_hex END. It is NOT COALESCE(emp.color_hex, asg.color_hex) — that
--   would borrow the assigned driver asg s colour onto an actual-driver emp who has no colour, i.e.
--   render a Jeffry-named avatar in Mark s colour. Verified: 0 rows where driver_color differs from
--   the driver_id person; Jeffry (uncoloured) now correctly NULL instead of Mark #F43F5E.
-- ops.v_calendar_driver.color_hex — for the filter legend / driver picker.
--
-- SAFE/ADDITIVE: both new columns appended last; the Calendar ignores them until it reads them, and
-- every other reader is unaffected. Verified: v_calendar_visit 1589 rows before = 1589 after.
-- AUDIT (ADR 010): view-only, no DML => no audit trigger applies.
-- REVERSIBLE: backups/2026-07-17_v_calendar_visit_before_drivercolor.sql +
--             backups/2026-07-17_v_calendar_driver_before_color.sql
...
CREATE OR REPLACE VIEW ops.v_calendar_visit AS  WITH last_completed AS (
 ...
 SELECT v.id,
    ...
    sagrp.sa_group,
        ...
        CASE WHEN emp.id IS NOT NULL THEN emp.color_hex ELSE asg.color_hex END AS driver_color
   FROM visits v
...
CREATE OR REPLACE VIEW ops.v_calendar_driver AS
 SELECT id, full_name, role, status, shift, phone, email, color_hex
   FROM employees
