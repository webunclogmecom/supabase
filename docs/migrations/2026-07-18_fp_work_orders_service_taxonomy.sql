-- 2026-07-18 — Field Portal work orders: Services list shows the canonical taxonomy (Fred)
--
-- WHY: `customer.work_orders.services` fed the client-facing FP work order with the RAW Jobber line-item
-- text (code prefix stripped). Clients saw the same service written many ways — "Grease Trap Pumping" (169),
-- "Service Agreement - Pumping - Grease Trap" (151), "…& Warranty of Drainnage" (61, Jobber typo),
-- "Service Agreement" alone (42, says nothing) — plus "Lyft station" typos. Fred asked for the same clean
-- taxonomy the Calendar now shows ([[reference_service_taxonomy_service_kind]]).
--
-- MAPPING (per line item, first match wins):
--   1. code prefix -> service_line_items.service_kind  (authoritative; covers ~57% of items)
--   2. conservative keyword fallback for free-typed Jobber names (~43% carry no code):
--        unclog -> Unclogging | pump -> Pumping | hydrojet -> Cleaning |
--        ^camera inspection -> Camera Inspection | dye test -> Dye Test | assessment -> Assessment
--   3. otherwise KEEP the item's own text.
-- Step 3 is deliberate: 63 distinct free-text names are genuine custom work / admin lines
-- ("Drain repair 4 inch. PVC", "Remove Toilet", "Concrete removal", "Clean out creation",
-- "GDO Online Reporting", "DERM manifests", "E-Report", "Service Agreement"). Forcing those into the
-- taxonomy would print a WRONG service on a customer receipt. Fred chose to KEEP admin lines visible.
-- Now DISTINCT (mapping collapses duplicates, e.g. two pumping line items -> one "Pumping").
--
-- Result: Pumping 572 / Unclogging 98 / Cleaning 91 / Labor 79 / Camera Inspection 16 /
-- Warranty of Drainage 14 / Dye Test 3 / Assessment 3 / Labor BUS 3, custom+admin lines unchanged.
--
-- SCOPE: ONLY the `services` expression changed. This view also carries other sessions' work — the
-- `decal` disposal-county expression and the FP-blackout `rd.url` / receipt_doc_class joins — which are
-- reproduced verbatim from the live definition. Diff before/after to confirm `services` is the only delta.
-- Frontend: the FP renders whatever `services` contains, so no Lovable change is needed.
--
-- AUDIT (ADR-010): view only, no audited table changes.
-- ROLLBACK: restore backups/2026-07-18_customer_work_orders_before_service_label.sql.
