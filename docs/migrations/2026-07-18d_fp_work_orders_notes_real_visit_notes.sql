-- 2026-07-18d — FP work orders: "Notes" shows the REAL visit note (and disappears when there isn't one)
--
-- WHY (Fred): "About the Notes, yes if there is no note for the visit you can hide it."
-- That rule could never fire, because the receipt's "Notes" was NOT the visit's note: the view mapped
-- `v.title AS notes` — the Jobber JOB TITLE — so it was populated on 552/552 work orders and rendered as
-- redundant noise ("081-TCE The carrot express … - Service Agreement - Grease Trap Pumping & Tank Cleaning"),
-- duplicating the client name + services already on the receipt.
--
-- FIX: notes now = NULLIF(TRIM(v.notes), '') — the actual visit note, with blank/whitespace normalised to
-- NULL. The Field Portal already renders this section conditionally (`{notes && (…)}` in WorkOrderView.tsx,
-- verified in source; no "—" placeholder), so an absent note now HIDES the whole block with **no frontend
-- change and no publish** — which matters because the FP Lovable project currently holds another session's
-- unpublished edits and must not be published (rule #13).
--
-- EFFECT (measured): of 552 work orders, 551 hide the Notes section and 1 shows a real note —
-- 168-AVA 2026-07-01 "run all hose in parking, ask AA for more instructions". Fred was shown that example
-- and chose "show real notes, hide when empty"; flagged to him since it is internal crew shorthand and is
-- the only note that becomes customer-visible (clearing it in Jobber removes it from the receipt).
--
-- NB `visits.notes` is genuinely sparse (852 of 860 completed visits have none) and is internal crew
-- shorthand when present — if that ever needs to be customer-safe, filter here, not in the app.
--
-- SCOPE: ONLY the `notes` expression changed; every other column (services / service_items / service_type,
-- the decal expression, the FP-blackout rd.url + receipt_doc_class joins) is reproduced verbatim from the
-- live definition. Column name, type (text) and position are unchanged.
--
-- AUDIT (ADR-010): view only. ROLLBACK: backups/2026-07-18d_customer_work_orders_before_notes_remap.sql
...
CREATE OR REPLACE VIEW customer.work_orders AS
 SELECT v.public_id AS id,
    ...
    NULLIF(TRIM(BOTH FROM v.notes), ''::text) AS notes,
