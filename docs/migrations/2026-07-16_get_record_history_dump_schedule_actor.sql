-- 2026-07-16_get_record_history_dump_schedule_actor.sql
-- Teach the Activity trail to name the DUMP Schedule driver instead of saying "System".
--
-- WHY (Fred, 2026-07-16, on a visit created from the truck QR): "it created the visit, i see it on
-- the Calendar App, but it didn't added a correct Activity." The drawer read "System created this
-- visit".
--
-- TWO causes, both fixed (the second one is here):
--   1. The edge fn sent no x-app-source header, so audit.log_change() fell back to app_source='sql'.
--      Fixed in supabase/functions/dump-visit-create/index.ts by setting the header on the client --
--      every other app already stamps itself (visit-calendar, derm-tracker, admin-review).
--   2. THIS function has a HARD-CODED allow-list of app_source -> actor_label. 'dump-schedule' was
--      not in it, so it fell through to the generic ELSE 'System' branch. Stamping the header alone
--      would therefore have changed NOTHING on screen -- both fixes are required.
--
-- THE ACTOR IS THE DRIVER, and he has NO auth identity (the app is no-login by design), so unlike
-- the other human apps we cannot resolve him from actor_email. He IS, however, the visit's own
-- assigned_driver_id -- so the label reads him off new_row and joins employees. Renders as:
--     "Anthony · DUMP app created this visit"
-- and actor_type='human', so the drawer shows a person rather than the system sync icon.
--
-- Only ONE entry appears per dump visit: the test-mode source/sync_state flip done by
-- public.create_dump_visit is invisible here, because this function only surfaces an UPDATE when it
-- touches a column in audit.entity_render_config, and neither source nor sync_state is one.
--
-- SAFE: the new branch is inserted immediately BEFORE the generic ELSE 'System'; every existing
-- app_source branch (visit-calendar/field-portal/derm-tracker/admin-review, jobber, reconcile/drift,
-- sa-cron, backfill, *-cron) is byte-identical. Signature and RETURNS TABLE are unchanged, so the
-- ops.get_record_history wrapper (which just forwards to this) needs no change.
-- AUDIT (ADR 010): read-only reporting function; no DML.
-- REVERSIBLE: backups/2026-07-16_get_record_history_before.sql
