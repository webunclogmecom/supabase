-- Migration: Calendar view — past/today shows Jobber only; cron projections future-only
-- Date: 2026-06-02
-- Author: Claude (for Fred)
-- Audit: N/A (view — no audit trigger)
-- Why (Fred 2026-06-02): the Calendar's past + today must reflect REAL visits (Jobber),
--   not cron-generated recurring projections. A projection is a prediction; once its date
--   is reached, only the actual Jobber visit should show. Future dates keep the projections
--   (the planned schedule). Time-based + automatic — no cleanup cron needed; aged projections
--   drop out of the view the moment their date is <= today (ET).
-- Also: the 2 stray cron projections already at/under today were soft-deleted in the same change.
...
CREATE OR REPLACE VIEW ops.v_calendar_visit AS  WITH last_completed AS (
 ...
 SELECT v.id,
  ...
  WHERE v.deleted_at IS NULL AND NOT (v.source = 'supabase_cron'::text AND v.visit_date <= (now() AT TIME ZONE 'America/New_York'::text)::date);
