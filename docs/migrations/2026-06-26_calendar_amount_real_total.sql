-- ============================================================================
-- 2026-06-26_calendar_amount_real_total.sql
-- ops.v_calendar_visit.amount: was COALESCE(service_config price, median historical)
-- = a PROJECTION, never the visit's real line-item total (off up to $50, dropped
-- cents on 777/1268 visits). Redefine amount = the visit's EFFECTIVE line-item
-- total (visit-scoped line_items, COALESCE to job-scoped for SA), numeric(12,2) --
-- matching ops.v_calendar_visit_detail's aggregation so list + drawer agree.
-- The old projection is preserved as a NEW trailing column amount_estimated
-- (for unpriced future visits; UI can fall back to it for $0). Additive: all 61
-- existing columns kept in order; amount_estimated appended at end (CREATE OR
-- REPLACE-safe). Frontend auto-corrects (it already binds amount + sums it for
-- day totals; SUM ignores NULLs). Rule 7: derived on read, not stored.
-- ============================================================================
CREATE OR REPLACE VIEW ops.v_calendar_visit AS
 ...
 SELECT v.id,
    ...
    COALESCE(
        ( SELECT sum(li.total_price) AS sum
               FROM line_items li
    ...
    COALESCE(sc.price_per_visit, op.median_line_price)::numeric(12,2) AS amount_estimated
   FROM visits v
