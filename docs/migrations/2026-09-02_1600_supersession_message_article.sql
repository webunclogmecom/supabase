-- 2026-09-02_1600_supersession_message_article.sql
--
-- WHAT: derm.record_page_rules picks the indefinite article from the grade instead of hardcoding
--       "a". One expression. No behaviour change.
--
-- WHY:  Fred read the refusal message and it says "this page already has a OK scan". The grade
--       vocabulary is OK / IRREGULAR / SPARSE / FAILED, so two of the four need "an", and OK is by
--       far the commonest one an operator will ever see: it is the message shown when a failed
--       re-measure is correctly refused on a page somebody has hand-measured.
--
--       Cosmetic, and worth doing anyway: this string is one of the few things the app shows a
--       person in the operator's own words rather than the detector's, and the whole point of the
--       2026-09-02 hint work was that those sentences read like they were written for a human.
--
-- THE BODY WAS COPIED FROM pg_get_functiondef AND EDITED BY ANCHOR, NEVER RETYPED.
--
...
DO $$
DECLARE
  v_res jsonb; v_etag text; v_n int; v_flag text := '';
  BAD constant jsonb := '[
...
BEGIN
  BEGIN
    -- Drive the REAL refusal on the page that actually carries an OK hand-marked scan, using its
    -- own etag, because the guard only fires when the image has not changed.
    ...
    IF v_etag IS NULL THEN
      RAISE EXCEPTION 'VERIFY SETUP FAILED: no hand-marked scan with an etag to refuse against';
    END IF;

    v_res := derm.record_page_rules('ticket-834489', 1, 'runlen-v2-article-probe',
    ...
    IF v_res->>'skipped' IS DISTINCT FROM 'would_supersede_ok' THEN
      RAISE EXCEPTION 'VERIFY 1 FAILED: the guard did not fire, so the message was never produced: %', v_res::text;
    END IF;
    IF position('has an OK scan' in (v_res->>'detail')) = 0 THEN
      RAISE EXCEPTION 'VERIFY 1 FAILED: message still reads %', v_res->>'detail';
    END IF;
    -- CONTROL: the old wording must be gone, or a passing check could be matching a substring
    IF position('has a OK scan' in (v_res->>'detail')) > 0 THEN
      RAISE EXCEPTION 'VERIFY 1 FAILED: the old wording survives: %', v_res->>'detail';
    END IF;

    v_flag := 'ok';
    RAISE EXCEPTION 'ROLLBACK' USING ERRCODE='ZZ021';
  EXCEPTION WHEN SQLSTATE 'ZZ021' THEN NULL;
  END;
  IF v_flag <> 'ok' THEN RAISE EXCEPTION 'VERIFY FAILED: probe did not complete'; END IF;

  -- 2. the consonant arm must still read "a", or the CASE is just a different hardcode
  IF format('%s', CASE WHEN left('SPARSE',1) IN ('A','E','I','O','U') THEN 'an' ELSE 'a' END) <> 'a' THEN
    RAISE EXCEPTION 'VERIFY 2 FAILED: SPARSE would take the wrong article';
  END IF;
  IF format('%s', CASE WHEN left('IRREGULAR',1) IN ('A','E','I','O','U') THEN 'an' ELSE 'a' END) <> 'an' THEN
    RAISE EXCEPTION 'VERIFY 2 FAILED: IRREGULAR would take the wrong article';
  END IF;

  -- 3. nothing survived
  SELECT count(*) INTO v_n FROM derm.page_rule_scans
   WHERE dump_folder='ticket-834489' AND source='runlen-v2-article-probe';
  IF v_n <> 0 THEN RAISE EXCEPTION 'VERIFY 3 FAILED: % probe scan(s) survived', v_n; END IF;

  RAISE NOTICE 'OK: the refusal now reads "an OK scan", and SPARSE still takes "a".';
END $$;
