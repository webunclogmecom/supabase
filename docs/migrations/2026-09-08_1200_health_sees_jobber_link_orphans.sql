-- ============================================================================
-- 2026-09-08_1200_health_sees_jobber_link_orphans.sql
--
-- Teach the daily Jobber sync health check to see a row that has NO Jobber link, because
-- nothing else could, and that is why "Bibi's burgers" sat in the Client App as four rows.
--
-- 🛑 THE POINT IS NOT THAT NOBODY LOOKED. EVERY EXISTING DETECTOR WAS BLIND BY SHAPE.
--    scripts/sync/weekly_dedup_audit.js runs every Sunday and has three detectors:
--      1. duplicate client_code -- filters client_code IS NOT NULL; an orphan's is NULL
--      2. duplicate address     -- joins properties; an orphan has none
--      3. stale Jobber GID      -- selects FROM entity_source_links, so it can ONLY see rows
--                                  that already HAVE a link. That is the exact complement of
--                                  the defect: it can never, by construction, report a row
--                                  whose entire problem is that it has no link.
--    Measured 2026-09-08 with positive controls: those three return 4 / 65 / 465 rows overall
--    (so all three are alive and none is merely misconfigured) and 0 of the 8 orphans. The audit
--    last ran 2026-09-06 and reported nothing at all while the orphans sat in the app.
--
-- ⚠ SO THIS IS DELIBERATELY KEYED ON THE LINK, NOT ON ANY COLUMN. A detector that starts from a
--   column value inherits the same blindness, because an orphan's distinguishing feature is an
--   ABSENCE. "Which rows have no entity_source_links row" is the only question that finds them.
--
-- ⚠ AND IT IS GENERIC OVER ENTITY TYPES ON PURPOSE. The client race is fixed as of
--   2026-09-08_1100, but the same three-request shape is still live in handleInvoice,
--   handleQuote, handleJob, handleProperty and handleVisit. Measured unlinked rows created since
--   the 2026-08-21 webhook enablement: invoices 57, quotes 9, clients 8 (now cleaned), jobs 0,
--   properties 0. Hard-wiring this to clients would have hidden the larger half.
--
-- ⚠ THE 2026-08-21 FLOOR IS A REGIME CHANGE, NOT A TIDY-UP DATE. Real Jobber webhooks began
--   working that day (the nested-payload fix in webhook-jobber), which put a second concurrent
--   driver alongside the */5 poll. Rows older than that are a separate, closed story: they are
--   counted as link_orphans_legacy so they never cause daily noise, and never silently vanish.
--
-- ⚠ THE SAMPLE IS CAPPED AT 25 PER ENTITY AND THE CAP IS REPORTED. link_orphans is the true
--   count, link_orphans_listed is how many reached items. A silent truncation reads as "that is
--   all of them", which is the same failure mode this whole check exists to prevent.
--
-- This replaces public.log_jobber_sync_health() in full (pg_get_functiondef output plus four
-- insertions). Nothing else changes: the diff is the declare block, one detector loop, three
-- keys in details, and one sentence of coverage text.
--
-- Audit: N/A (one function replace).
-- ============================================================================
...
do $verify$
declare
  v_ret int; v_details jsonb;
  v_cl int; v_listed int; v_legacy int; v_total int; v_inv_items int;
  v_inv_expected int; v_qt_expected int;
begin
  -- INDEPENDENT MEASUREMENT FIRST, so the detector is compared against a number it did not produce.
  select count(*) into v_inv_expected from public.invoices i
   where not exists (select 1 from public.entity_source_links e
  ...
  select count(*) into v_qt_expected from public.quotes q
   where not exists (select 1 from public.entity_source_links e
  ...
  -- 🛑 POSITIVE CONTROL. If there were nothing to find, a zero from the detector would be
  --    untestable and this VERIFY would pass vacuously. Refuse rather than pass.
  if v_inv_expected = 0 and v_qt_expected = 0 then
    raise exception 'VERIFY 0 FAILED: no known orphans exist, so the detector cannot be tested here. Re-check by hand.';
  end if;

  v_ret := public.log_jobber_sync_health();

  select l.details into v_details from public.sync_log l
   where l.sync_source = 'jobber-sync-health' order by l.started_at desc limit 1;
  if v_details is null then
    raise exception 'VERIFY 1 FAILED: the check wrote no sync_log row -- it is broken, and broken silently';
  end if;

  v_total  := (v_details->>'link_orphans')::int;
  v_listed := (v_details->>'link_orphans_listed')::int;
  v_legacy := (v_details->>'link_orphans_legacy')::int;
  if v_total is null or v_listed is null or v_legacy is null then
    raise exception 'VERIFY 2 FAILED: the new keys are missing from details: %', left(v_details::text, 400);
  end if;

  -- 3. THE DETECTOR AGREES WITH THE INDEPENDENT COUNT. This is the assertion that matters:
  --    a detector reporting 0 while 57 orphans exist is precisely the failure being fixed.
  if v_total < v_inv_expected + v_qt_expected then
    raise exception 'VERIFY 3 FAILED: detector reports % orphans but invoices+quotes alone are % -- it is BLIND',
      v_total, v_inv_expected + v_qt_expected;
  end if;

  -- 4. and they actually reached items, where ops.v_health_items can render them
  select count(*) into v_inv_items
    from jsonb_array_elements(coalesce(v_details->'items','[]'::jsonb)) i
   where i->>'issue' = 'jobber_link_orphan' and i->>'entity_type' = 'invoice';
  if v_inv_items = 0 then
    raise exception 'VERIFY 4 FAILED: % invoice orphans counted but none reached items', v_inv_expected;
  end if;

  -- 5. the cap is honest: listed can never exceed the true count
  if v_listed > v_total then
    raise exception 'VERIFY 5 FAILED: listed % exceeds the true count %', v_listed, v_total;
  end if;

  -- 6. CLIENTS ARE NOW ZERO. The 1100/1130 pair worked, and the detector can still see clients --
  --    proven by the invoice arm above, which runs the identical generated code path.
  select count(*) into v_cl
    from jsonb_array_elements(coalesce(v_details->'items','[]'::jsonb)) i
   where i->>'issue' = 'jobber_link_orphan' and i->>'entity_type' = 'client';
  if v_cl <> 0 then
    raise exception 'VERIFY 6 FAILED: % client orphan(s) still present after the 1130 cleanup', v_cl;
  end if;

  -- 7. the pre-existing half of the check survived (do not trade one blindness for another)
  if v_details->'covers' is null or v_details->>'covers' not like '%13 columns of 93%' then
    raise exception 'VERIFY 7 FAILED: the original coverage statement was lost';
  end if;
  if v_details->'drift_client_name' is null then
    raise exception 'VERIFY 7b FAILED: an existing counter disappeared from details';
  end if;

  raise notice 'VERIFY ok: detector agrees with an independent count (invoices %, quotes %); items populated; cap honest; clients now 0; original coverage and counters intact',
    v_inv_expected, v_qt_expected;
end
$verify$;
