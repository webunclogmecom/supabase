-- 2026-07-01_service_kind_per_job_cadence.sql
-- FIX: service_kind mislabeled one-off Service Calls as 'SA', leaking them into "SA visits"
-- panels. service_kind used TWO client-level signals as a per-job proxy: sc.frequency_days
-- (service_configs, per client+service_type) and oc.median_gap_days (median visit gap, also
-- per client+service_type). So any one-off SC for a client who ALSO has recurring service of
-- the same type inherited that client's cadence -> non-zero "frequency" -> tagged SA.
-- Now service_kind is resolved per-JOB, in priority order:
--   1. real Jobber frequency_days > 0            -> SA (authoritative recurring schedule)
--   2. title ~ 'service call' / 'emergency'      -> SC (explicit one-off; OVERRIDES cadence,
--      because pre-canonicalization '[OLD]' jobs lump many separate calls under one job_id,
--      giving a service-call job a fake multi-visit "cadence")
--   3. per-JOB median gap (5-200d) > 0, OR title ~ grease / grey water / service agreement -> SA
--   4. else                                      -> SC
-- Verified: genuine Service Calls (incl. [OLD] aggregates) -> SC, 0 recurring-title regressions,
-- 0 real-SA losses. The DISPLAY frequency_days + late_status intentionally KEEP the client-level
-- cadence (sc.frequency_days + oc.median_gap_days) and are unchanged.
 ...
 SELECT v.id,
    ...
    v.start_at IS NULL OR (v.start_at AT TIME ZONE 'America/New_York'::text)::time without time zone = '00:00:00'::time without time zone AND v.end_at IS NOT NULL AND (v.end_at - v.start_at) >= '23:00:00'::interval AS is_all_day,
        CASE
            ...
            WHEN lower(jb.title) LIKE '%service call%' OR lower(jb.title) LIKE '%emergency%' THEN 'SC'::text
            WHEN ojc.median_gap_days > 0
              OR lower(jb.title) LIKE '%grease%'
              OR lower(jb.title) LIKE '%grey water%'
              OR lower(jb.title) LIKE '%service agreement%' THEN 'SA'::text
            ELSE 'SC'::text
        END AS service_kind,
    v.notes,
    v.sync_state
