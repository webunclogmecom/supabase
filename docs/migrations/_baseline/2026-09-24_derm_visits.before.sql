-- Baseline (pre-change) definition of derm.visits, before 2026-09-24_1600_derm_visits_grey_water_pumping.sql (md5 39268bc36c7b38adff2b803d6eea85ef).
-- Rollback: CREATE OR REPLACE cannot drop the added column, so DROP VIEW derm.visits (no dependents), run this file,
-- then restore the ACL: GRANT SELECT ON derm.visits TO authenticated; GRANT ALL ON derm.visits TO service_role.
-- The DERM Tracker list must stop selecting grey_water_pumping first. Unqualified names resolve on the default path.
