-- 2026-08-03_0430  ops.v_calendar_visit: expose zone_color for the Calendar visit drawer
--
-- Yannick's drawer redesign asks for "the color bubble around the zone name". The colours already
-- existed -- public.zones carries color_hex for all 11 zones -- but this view exposed only zone
-- (the CODE), so the Calendar had nothing to colour with. This is the ONLY backend gap in his list:
-- expected_date, equipment_size_gallons, start_at, frequency_days and job_id are already here.
--
-- Mirrors the existing zone expression exactly. zone is COALESCE(pz.code, ppz.code) over the two
-- LEFT JOINs to zones already in this view (visit property, then client primary property), so
-- zone_color is COALESCE(pz.color_hex, ppz.color_hex) and cannot disagree with the code it belongs to.
--
-- MEASURED BEFORE WRITING:
--   all 11 zone codes in live use join to a zones row, including MIAMI BEACH, MID/EDG and SF/BH
--   AVE #C0F0F0 · BRO #FFEAB6 · DOWN #FFDCE5 · MIAMI BEACH #FFDAF6 · MID/EDG #9CC7FF
--   NMB #D1F7C4 · PALM #ECECEC · SF/BH #FEE2D5 · SOUTH #2D7FF9 · SUNNY #EDE2FE · WEST #77D1F3
--   zone itself is set on 915 of 967 recent visits, so ~5% legitimately have NO zone and must
--   render with no bubble at all rather than a grey one.
--
-- PROBED rolled back: column list is exactly the old list + ',zone_color' (appended_only = true),
-- 71 -> 72 columns, and ZERO rows where zone IS NOT NULL AND zone_color IS NULL.
--
-- ⚠ The check that matters was done against information_schema, not by regexing the view SQL.
-- My first attempt asserted the column order with a regex over pg_get_viewdef and it reported a
-- false failure, because ' AS <alias>' also matches aliases inside the subqueries and lateral
-- joins that follow the select list. CREATE OR REPLACE VIEW rejects a changed column list (42P16)
-- but would silently accept a REORDER, so ask the catalogue, not the text.
--
