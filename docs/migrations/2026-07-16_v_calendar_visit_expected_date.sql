-- 2026-07-16_v_calendar_visit_expected_date.sql
-- Add expected_date to ops.v_calendar_visit: "the date this visit SHOULD be" = previous visit + frequency.
--
-- WHY (Yan, 2026-07-16): "we wanted in the drawer to have an information which is the date the visit
-- should be (past visit + frequency)". The Calendar visit drawer shows the actual DATE & TIME but not
-- what the cadence implies, so you cannot see that e.g. 032-LG La Granja 36th St ran on Jul 3 when it
-- was due Jun 28.
--
-- ⚠ IT REUSES THE LATENESS ANCHOR VERBATIM - THIS IS THE WHOLE POINT. The view already computes this
-- expression inside late_status. Deriving the drawer date independently would let the tooltip and the
-- red "Late" chip contradict each other on the same visit. Same prev_live anchor (the previous
-- completed-or-scheduled, non-deleted visit for the same client+service_type, before this visit_date)
-- and the same frequency fallback chain: jobs.frequency_days -> service_configs.frequency_days ->
-- observed_cadence.median_gap_days (alias oc, NOT ojc/observed_job_cadence which service_kind uses).
-- Verified: 0 rows where late_status and expected_date disagree.
--
-- DELIBERATE DIFFERENCE FROM late_status: late_status is NULL for completed/skipped visits, but
-- expected_date is NOT - a completed visit still has a meaningful "was due Jun 28, ran Jul 3", which
-- is exactly the case Fred screenshotted. expected_date is NULL only when there is no previous visit
-- or no frequency to derive from (349 of 1587); the app hides the icon then.
--
-- ⚠ RENDER RAW: expected_date is a DATE (no time). Per [[project_apps_display_in_ET]], date-only
-- values must be rendered RAW - passing it through a Date/timezone conversion shifts it a day for a
-- viewer west/east of ET (our reviewer is in Paris, so the bug would look fine to him).
--
-- SAFE/ADDITIVE: all existing columns keep name/type/ORDER; expected_date appended LAST.
-- Verified: 1587 rows before = 1587 after; 1238 have an expected_date.
-- AUDIT (ADR 010): view-only, no DML => no audit trigger applies.
-- REVERSIBLE: backups/2026-07-16b_ops_v_calendar_visit_before_expected.sql
 ...
 SELECT v.id,
    ...
    sagrp.sa_group,
        CASE
            ...
            WHEN COALESCE(NULLIF(jb.frequency_days, 0), sc.frequency_days, oc.median_gap_days) IS NULL THEN NULL::date
            ELSE (pl.prev_live_date + COALESCE(NULLIF(jb.frequency_days, 0), sc.frequency_days, oc.median_gap_days)::double precision * '1 day'::interval)::date
