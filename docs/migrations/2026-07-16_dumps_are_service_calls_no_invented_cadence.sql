-- 2026-07-16_dumps_are_service_calls_no_invented_cadence.sql
-- Disposal sites are Service Calls, and "supposed to be" now requires a DECLARED cadence.
--
-- WHY (Fred, 2026-07-16, on the Homestead Dump drawer): "the dump places are Service Calls not
-- Service Agreement, even on jobber it shows as a normal job we need to fix this. Also why does it
-- shows a 'Supposed to be' icon when it's not a SA client, it has no recurrent visits so there is no
-- logic behind a 'Supposed to be' time."
--
-- ROOT CAUSE — one invented number, three visible lies. ops.v_calendar_visit derived
-- frequency_days as COALESCE(jobs.frequency_days, service_configs.frequency_days,
-- observed_cadence.median_gap_days). That LAST fallback measures the OBSERVED gap between past
-- visits, so because the crew dumps roughly every 9 days, the Homestead dump was handed a
-- frequency of 9 days that nobody ever agreed to. Then:
--   1. service_kind: job 1720 (000-DH) has a NULL title and frequency_days=0, so the SA/SC CASE fell
--      past its title tests to `WHEN ojc.median_gap_days > 0 THEN 'SA'` -> the drawer chipped a
--      disposal site as a "Service Agreement".
--      (000-DP was correct only by luck -- its job happens to be titled "Service Call".)
--   2. frequency_days: the drawer would report "Every 9 days" for a dump.
--   3. expected_date: prev_visit + that invented 9 days -> a "Supposed to be: Jul 24, 2026" on a
--      client that has no agreement and no cadence at all.
--
-- THE FIX — two rules, both at the root:
--   A. A DISPOSAL SITE IS NEVER AN AGREEMENT. client_code LIKE '000-%' (exactly two clients exist:
--      365 '000-DH' Homestead Dump and 76 '000-DP' DUMP Pompano -- both are our own facilities, not
--      customers) => service_kind='SC', frequency_days = the DECLARED value only (so NULL for the
--      dumps), expected_date = NULL. This is a domain fact, not a heuristic: we do not hold a
--      service agreement with our own dump.
--   B. "SUPPOSED TO BE" REQUIRES A DECLARED CADENCE. expected_date now uses
--      COALESCE(NULLIF(jobs.frequency_days,0), service_configs.frequency_days) and NO LONGER falls
--      back to observed_cadence.median_gap_days. A date the customer agreed to is a fact; the mean
--      gap between past visits is an observation, and presenting an observation as "supposed to be"
--      is exactly the "no logic behind it" Fred called out. No declared cadence => no icon.
--
-- DELIBERATELY NOT CHANGED: late_status still uses the observed median in its fallback. That is a
-- separate, pre-existing behaviour with its own tuning history (2026-07-03 per-visit anchor), and
-- narrowing it here would silently change which chips read red across the whole calendar. The two
-- stay consistent where both exist: verified 0 rows where late_status and expected_date disagree.
--
-- VERIFIED: 1589 rows before = 1589 after. Dump visits: 0 remain SA / with an invented frequency /
-- with a supposed-to-be. No regression: 032-LG stays SA freq=10 expected=2026-06-28; 195-MYK keeps
-- its PUMPING_GT / CLEANING colours and its expected dates. Coverage 1238 -> 1171 expected_date: the
-- 67 dropped were built on an observed average only. The 51 Service Calls that KEEP an expected_date
-- have real declared service_configs cadences (168-AVA 30d, 075-TCE 120d, 021-GRA 364d) -- they are
-- recurring clients, so a supposed-to-be there does have logic.
--
-- AUDIT (ADR 010): view-only, no DML => no audit trigger applies.
-- REVERSIBLE: backups/2026-07-16c_v_calendar_visit_before_dump_sc.sql
