-- Migration: revert "past/today = Jobber only" calendar rules (app owns visits now)
-- Date: 2026-06-02
-- Author: Claude (for Fred)
-- Why (Fred 2026-06-02): "remove past visits, keep Jobber" was a ONE-TIME cleanup, not an
--   ongoing rule. With the new model (Calendar app owns the full visit lifecycle; Jobber
--   visit-inbound stopped), past/today visits come from the app too. So we (1) unschedule
--   the daily pg_cron purge, and (2) revert ops.v_calendar_visit to deleted_at-only (show
--   ALL app visits, every date). Supersedes 2026-06-02_calendar_past_jobber_only.sql and
--   2026-06-02_purge_past_cron_visits_pgcron.sql.
-- Audit: N/A (view + cron schedule).

DO $$ BEGIN PERFORM cron.unschedule('purge-past-cron-visits'); EXCEPTION WHEN OTHERS THEN NULL; END $$;
