-- Migration: 2026-07-03f_calendar_views_skip_reason.sql
-- Author: Claude (Supabase session)
-- Expose visits.skip_reason on the two Calendar app views (append-only, last column) so the
-- Visit Calendar drawer can display WHY a visit was skipped and the grid can tooltip it.
-- Companion to 2026-07-03c/d/e (skip-a-visit); required by the shipped Skip/Un-skip UI.
-- Bodies are the live definitions + the one appended column each.
BEGIN;
...
CREATE OR REPLACE VIEW ops.v_calendar_visit_detail AS  SELECT v.id AS visit_id,
    v.job_id,
    v.client_id,
    v.title,
    v.visit_date,
