-- 2026-07-10_fp_blackout_wwtp_hide.sql
-- Fred decision 2026-07-10: HIDE the raw white-manifest photo from the customer portal until a
-- redacted version exists. customer.work_orders.wwtp_receipt_url previously served dm.derm_manifest_url
-- (the UNREDACTED white FOG form photo — Section B can name up to 5 co-client facilities; flagged by
-- the Blackout adversarial review). Now NULL::text. wwtp_receipt_number/disposal_facility columns keep
-- working; the FP WWTP card must render its no-doc state when the url is NULL (app change alongside
-- the FOG card flip). Restore later by extending the redaction pipeline to the white form.
-- Also per Fred: customer.* anon-enumeration ACCEPTED for now (RPC-gating = future project).
