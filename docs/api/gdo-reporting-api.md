...
> This is the schema doc referenced in the integration handoff. It documents the **current** contract
> of the two endpoints your bot talks to, plus the surrounding pieces (event push, status surfaces,
> evidence storage, error codes). Nothing here is a secret: key **values** are sent to you privately,
...
JWT** and no database connection: the key is the auth, checked inside each function. The gateway is set
to `verify_jwt = false` for exactly this reason.

- The key is matched against a **comma-separated** secret on our side, so we can rotate with zero
  downtime (add new, you switch, we remove old). We will coordinate a rotation, you never see the list.
...
Send us your `/run` URL and we flip the push on (we store `rpa_run_url` + `rpa_run_key` in our config).
Until then the push is dormant and the poll backup alone already works.
...
- **Watchdog:** `public.v_rpa_derm_health` + `log_rpa_derm_health()` + a daily `rpa-derm-health-check`
  cron surface a stuck queue / repeated data errors / storage failures so the log is not write-only.
- **Launch cutoff:** `public.rpa_launch_cutoff()` (currently `2026-07-21`) bounds the live queue.
- **Evidence retention:** screenshots live in the private `rpa-evidence` bucket keyed `visit_id/run_id.jpg`.
- **PII note (under review):** the queue currently signs the **raw multi-client** address sheet (the
  full roster our customer-facing "FOG Manifest" redacts). This is intended because county submission
  needs the real document, but it is a wider exposure than customers get. If the portal only needs the
  client's own row, we will serve the redacted copy instead — pending confirmation of what the portal
  requires.
...
- Endpoints: `supabase/functions/rpa-derm-queue/index.ts`, `supabase/functions/rpa-derm-result/index.ts`.
- Backend + hardening: `docs/migrations/2026-07-21e_rpa_derm_portal_backend.sql`,
  `2026-07-21f_rpa_backend_hardening.sql`, `2026-07-21g_gdo_online_reporting_scope.sql`.
- Integration handoff (the human framing sent to John): `docs/handoffs/2026-07-21_rpa_bot_reply_to_john.md`.
- Postman test harness: `postman/` (dry-run queue + result flows).
