...
8. [Recurring visit generation (Supabase-native)](#8-recurring-visit-generation-supabase-native)
...
## 8. Recurring visit generation (Supabase-native)

The recurring visit schedule for `ACTIVE` / `Recuring` clients is generated by a Supabase-native daily cron — `scripts/sync/cron_generate_recurring_visits.js`, runnable via `.github/workflows/generate-recurring-visits.yml`. This replaces the legacy Airtable button-triggered automation as part of the May–June 2026 sunset (see [ADR 015](decisions/015-supabase-native-recurring-visits.md)).
...
# with dry-run (no DB writes, just reports what it would do):
gh workflow run generate-recurring-visits.yml -f dry_run=true
...
For every `ACTIVE` or `Recuring` client × every `service_configs` row with `frequency_days > 0`:

1. Compute the chain anchor (precedence): `MAX(scheduled.visit_date) + freq` → `MAX(completed.visit_date) + freq` → `service_configs.first_visit` → `today + freq`.
2. Generate candidate dates forward until **both** are satisfied: (a) end of next calendar month reached, (b) at least 1 future visit exists for that client+service.
3. For each candidate, check `visits` for any row at `(client_id, service_type, visit_date ±7 days, visit_status IN scheduled/late/today/completed)` — if found, skip. Idempotent: never duplicates.
4. INSERT new rows with `source='supabase_cron'`, `visit_status='scheduled'`, `title='<client_code> - Scheduled <service_type>'`.
5. Write one `sync_log` row per run with totals (job_name `generate_recurring_visits`).

### How to enable the daily schedule

The schedule is **disabled by default** until Yannick's "upcoming visits per client" view ships in Lovable. To activate:

1. Edit `.github/workflows/generate-recurring-visits.yml`, uncomment the `schedule:` block.
2. Commit + push.
3. **Same day**, tell ops to stop clicking the Airtable "Generate Visits" button. The cron is now the canonical source of the schedule.

### Interaction with Jobber + the merge logic

The cron writes rows with `source='supabase_cron'`. When a Jobber visit later arrives via `webhook-jobber.handleVisit` matching one of those rows by `(client_id, service_type, visit_date ±7 days)`, the existing row is **PROMOTED** (UPDATEd in place; `source` becomes `'jobber'`; Jobber `entity_source_link` attached). Single row per logical visit; no duplicates.

After Jobber sunset (June 2026), nothing arrives to promote and the merge logic becomes a no-op. Safe to leave or delete.

### INACTIVE / PAUSED clients

A Postgres trigger (`trg_clients_wipe_upcoming_on_inactive`) auto-deletes a client's scheduled future visits on `status` transition into `INACTIVE` or `PAUSED`. The cron filters them out at SELECT time too (belt + suspenders).

### If something looks wrong

```sql
-- 1. Last cron run + outcome:
SELECT * FROM sync_log WHERE job_name='generate_recurring_visits' ORDER BY started_at DESC LIMIT 5;

-- 2. Coverage check — how many active recurring clients have a scheduled future visit?
SELECT sc.service_type, COUNT(DISTINCT c.id) AS clients, COUNT(DISTINCT v.id) AS scheduled_future
FROM clients c
JOIN service_configs sc ON sc.client_id = c.id
LEFT JOIN visits v ON v.client_id = c.id AND v.service_type = sc.service_type
                    AND v.visit_status = 'scheduled' AND v.visit_date >= CURRENT_DATE
WHERE c.status IN ('ACTIVE','Recuring') AND sc.frequency_days > 0
GROUP BY sc.service_type ORDER BY sc.service_type;

-- 3. Source breakdown of scheduled visits:
SELECT source, COUNT(*) FROM visits
WHERE visit_status='scheduled' AND visit_date >= CURRENT_DATE
GROUP BY source ORDER BY COUNT(*) DESC;

-- 4. Orphan supabase_cron placeholders past their date (cron didn't get promoted by Jobber):
SELECT * FROM visits
WHERE source='supabase_cron' AND visit_status='scheduled' AND visit_date < CURRENT_DATE - INTERVAL '14 days';
-- Safe to delete:
-- DELETE FROM visits WHERE source='supabase_cron' AND visit_status='scheduled' AND visit_date < CURRENT_DATE - INTERVAL '14 days';
```
...
| 2026-04-21 | Samsara webhook audit — 12 webhooks registered, 6 pairs of duplicates; all signature-verification failed | Re-running `scripts/webhooks/register-samsara.js` created duplicate registrations (one per event type) without detecting the existing ones. Each Samsara webhook has its own signing secret → we only have 1 in `SAMSARA_WEBHOOK_SECRETS`, so only events signed with that specific secret verify | Fred to deduplicate via Samsara dashboard (keep 1 per event type), capture each kept webhook's secret, set `SAMSARA_WEBHOOK_SECRETS=secret1,secret2,…` (comma-separated list already supported by the Edge Function) | Webhook registration scripts must check existing registrations and be truly idempotent — not fire-and-forget |
| 2026-04-21 | Audit found 13 orphan Storage objects (files in bucket without a `photos` row) | Jobber migration uploaded attachments before DB-inserting the `photos` row. On per-note transaction failure (DNS crash, 503), the file stayed but the DB row didn't. | Deleted via a one-shot diff script (storage.objects LEFT JOIN photos WHERE photos.storage_path IS NULL) | Future migrations: upload-then-insert remains the right order (upload failures are cheap; DB failures without upload would leave the note pointing at nothing). Accept orphans as a known outcome + cleanup script |
